...
public final class ABIDecoder {
    ...
    public func decode(type: ABIType) throws -> ABIValue {
        switch type {
        case .uint(let bits):
            return .uint(bits: bits, try! decodeUInt())
        case .int(let bits):
        ...
        case .ufixed(let bits, let scale):
            return .ufixed(bits: bits, scale, try! decodeUInt())
    ...
    public func decodeBool() -> Bool {
        return try! decodeUInt() != BigInt(0)
    }

    /// Decodes an unsigned integer.
    public func decodeUInt() throws -> BigUInt {
        if offset + ABIEncoder.encodedIntSize > data.count {
    ...
    public func decodeBytes() -> Data {
        return decodeBytes(count: Int(try! decodeUInt()))
    }
